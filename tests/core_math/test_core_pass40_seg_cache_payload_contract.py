...
def test_segment_cache_store_validates_segment_payload_before_append() -> None:
    ...
    assert "seg_cache_segment_valid" in src
    assert "seg_cache_segments_valid" in src
    assert "!spectral_is_finite_f32(s->omega) || s->omega < 0.0f" in src
    assert "if (!seg_cache_segments_valid(sa->segs, sa->count, NULL))" in src

    validation_idx = src.index("if (!seg_cache_segments_valid(sa->segs, sa->count, NULL))")
...
def test_segment_cache_lookup_validates_mmap_segment_and_gpu_payload() -> None:
    ...
    assert "seg_cache_gpu_segments_match" in src
    ...
    validate_idx = src.index("seg_cache_gpu_segments_match(mapped_segments, mapped_gpu_segs")
...
def test_segment_cache_lookup_validates_heap_payload_after_endian_swap() -> None:
    ...
    assert "if (!seg_cache_segments_valid(segs, e->seg_count, NULL))" in src
...
def test_segment_cache_tile_layout_references_are_validated_before_publication_or_store() -> None:
    ...
    assert "seg_cache_tile_layout_words_valid" in src
    ...
    assert "!seg_cache_tile_layout_words_valid(tile_ranges," in src
