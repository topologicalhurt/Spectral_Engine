...
| bandwidth-01 dead LUT on M7 | bandwidth | med | **✅ LANDED** | The M7 coupled osc never reads the 8 KB sine LUT, yet `process()` required it non-null and the device/harness always inited it — a QEMU profile measured the dead init at **8.7% of the run** (`sinf` 6.5% + `spectral_lut_init_sine` 2.3%). Fixed: gated the `process()` precondition clause `|| !ctx->osc_lut` under `#if !SPECTRAL_ARM_M7`; gated `s_osc_lut` alloc + init + the init-arg in `daisy_seed_spectral.c` AND both QEMU harnesses (`qemu_main.c`/`qemu_render_main.c` now pass NULL on M7); dropped `spectral_lut_init_sine` from the counts rig's RANGE_SYMBOLS (gc-sections now removes it). **Byte-identical** (golden `7ed6e0ac` unchanged — the M7 never read it); device reclaims ~8 KB SRAM + the boot init. The LUT path stays intact for non-M7 builds (the user's "some runs use a LUT"); a LUT-on-M7 *runtime* option would need a dispatch flag (not added — the dispatch is compile-time `#if SPECTRAL_ARM_M7`). m7 baseline re-signed (labels shifted; SAMPLES_PER_ITER re-pinned). |
| **amp-pack** (pair kernel) | fpu-alu | med | **✅ LANDED (−16% process_insns, byte-identical)** | `spectral_qadd16` used the PACKED dual-16 `QADD16` for a SINGLE q15 (wasting the high lane + a sign-extend), and the pair kernel did TWO of them/sample while `spectral_smlald` repacked `(aA,aB)` every call. Now both voices' amps ride PACKED in one register: one `QADD16` (`spectral_qadd16x2`) ramps the pair, and `spectral_smlald_packed` consumes the packed amps directly. **process_insns 1,598,063 → 1,341,035 (−16%)**; pair kernel 38→32 insns/iter. The CYCLE win is only ~3% (16.0 → 15.5 cyc/voice-sample) — the removed adds were largely latency-hidden — but the instruction/energy drop is real. Bit-identical (golden unchanged); the standalone llvm-mca probe over-stated this at 18% (the inlined production is the truth). |
| accum-zero fold | bandwidth | low | **HELD (correctness edge)** | Folding the per-block accum zeroing into the output pack (`accum[j]=0` as it consumes) only clears `[0..num_samples]`, which breaks if a later block is LARGER (host tests render variable 256-blocks; the device is fixed 48/64). The robust per-block `memset` avoids that, and the device accum is already only 64-block (accum-block-cap reclaim), so the value is small and the clean form is risky. A write-first kernel variant is correct but doubles the kernel code for a per-block fixed cost that is negligible at 512 voices. Not landed. |
