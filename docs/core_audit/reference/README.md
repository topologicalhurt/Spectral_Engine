...
`docs/core_audit/` (the parent) holds **only the actively-worked plans**: `../REVIEWER_HANDOFF.md`
(the forward mandate) and the live workstreams (`../M7_PERF_MODEL_PLAN.md`,
`../IFFT_SYNTHESIS_PLAN.md`). Completed/superseded campaigns are in `../archive/`. Everything that
is **live but not the current focus** lives here.
