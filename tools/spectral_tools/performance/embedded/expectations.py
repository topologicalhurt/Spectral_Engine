...
def _live_numbers(tc: Toolchain, *, out_dir: Path,
                  verify_reproducible: bool = False) -> dict[str, Any]:
    ...
    counts = measure(tc, out_dir=out_dir,
...
def generate(tc: Toolchain, *, out_dir: Path) -> Path:
    ...
    Generation runs the reproducibility double-run: a frozen golden must be
    proven deterministic at signing time. verify() skips it — it runs per
    commit, and determinism was the generate-time guarantee."""
    live = _live_numbers(tc, out_dir=out_dir, verify_reproducible=True)
    doc = {
...
def compare(base: dict[str, Any], live: dict[str, Any]) -> list[str]:
    ...
    if base_params != live_params:
        ...
        # Element-compare only over a matching set; on drift the pairing is
        # meaningless and the gate has already failed by name above.
