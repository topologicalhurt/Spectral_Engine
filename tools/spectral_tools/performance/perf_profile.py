...
def _with_backend(cli_args: list[str], backend_id: int) -> list[str]:
    """Override the ``backend_id`` positional argument in *cli_args*.

    The positional contract is defined by :data:`PERF_ARG_NAMES` / :data:`DEFAULT_PERF_CLI_ARGS`.
    """
    args = list(cli_args)
    defaults = DEFAULT_PERF_CLI_ARGS
    if len(args) < len(defaults):
        args = args + defaults[len(args) :]
    args[_BACKEND_IDX] = str(backend_id)
    return args
