...
class BranchFormatter:
    ...
    def _status_text(self, raw: str) -> str:
        ...
        if status == "ok":
            return self.console.green("ok")
        if status == "failed":
            return self.console.red("FAILED")
        if status == "skipped":
            return self.console.yellow("skipped")
        if status == "blocked":
            return self.console.yellow("blocked")
        return raw

    def _perf_status_text(self, raw: str) -> str:
        norm = (raw or "").strip().lower()
        if norm == "ok":
            return self.console.green("ok")
        if norm == "failed":
            return self.console.red("FAILED")
        if norm in {"blocked", "unavailable"}:
            return self.console.yellow(norm)
        if norm == "skipped":
            return self.console.yellow("skipped")
        return raw
        ...
        if perf_statuses:
            ...
            for label in self.PERF_STATUS_ORDER:
                if label in perf_statuses:
                    status_parts.append(f"{self._perf_status_text(label)}={perf_statuses[label]}")
            for label in chain(perf_statuses.keys()):
        ...
        for test in tests:
            ...
            if isinstance(details, dict):
                ...
                if isinstance(perf_profile, dict):
                    ...
                    perf_tag = f"  {self.console.cyan('-P')}={self._perf_status_text(perf_state)}"
