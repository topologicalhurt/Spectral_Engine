...
def generate_c(entries: list[ResourceHashEntry]) -> str:
    ...
    lines.append("/* Hash algorithms: host=xxh3_64, embedded=xxh32 */")
    lines.append("/*")
    lines.append(" * Note:")
    ...
    for entry in entries:
        lines.append(
            f'    {{ UINT32_C(0x{entry.file_id:08x}), UINT64_C(0x{entry.embedded_digest:016x}), (size_t){entry.size_bytes}u }},'
        )
    lines.append("#else")
    for entry in entries:
        lines.append(
...
def main() -> None:
    ...
    if xxhash is not None:
        ...
        print(f"Using xxhsum CLI ({xxhsum_bin}) for host; xxh32 for embedded")

    entries = collect_entries(resources_dir, xxhsum_bin)
